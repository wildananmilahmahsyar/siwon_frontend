...
const Admin = () => {
  ...
  const fetchHewan = async () => {
    try {
      ...
      const res = await fetch(
        `${import.meta.env.VITE_API_BASE}/api/hewan?adminId=${
      ...
      setHewanList(data.filter((h) => !h.isAdopted));
    } catch (err) {
  ...
  useEffect(() => {
    if (user?.id) {
      fetch(
        `${import.meta.env.VITE_API_BASE}/api/pengajuan/menunggu?adminId=${
          user.adminId || user.id
        }`,
        {
          headers: {
        ...
        .then((data) => {
          ...
          data.forEach((item) => {
            ...
            console.log(
              "Foto:",
              item.foto ? item.foto.substring(0, 50) : "tidak ada foto"
            );
          });
  ...
  const handleCheckboxChange = (e) => {
    ...
    setFormData((prev) => {
      const kondisi = checked
        ? [...prev.kondisi, value]
        : prev.kondisi.filter((k) => k !== value);
      return { ...prev, kondisi };
    });
  };
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      if (res.ok) {
        Swal.fire({
          icon: 'success',
          title: editingHewan ? 'Data berhasil diperbarui!' : 'Data berhasil ditambahkan!',
  ...
  const handleVerifikasi = async (id, status) => {
    if (
      !window.confirm(
        `Yakin ingin ${
          status === "diterima" ? "menerima" : "menolak"
        } pengajuan ini?`
      )
    )
      return;
    ...
    <div className="admin-container">
      ...
      <main className="admin-content">
        <section className="admin-card">
          ...
          <button className="btn-edit" onClick={togglePopup}>
            MASUKKAN HEWAN
          </button>
              ...
              <div className="popup-form" onClick={(e) => e.stopPropagation()}>
                ...
                <div className="popup-content">
                    ...
                    {[
                      ...
                      <label key={idx}>
                        <input
                          type="checkbox"
                          value={item}
                          checked={formData.kondisi.includes(item)}
                  ...
                  <button className="upload-btn" onClick={handleSubmit}>
                    {editingHewan ? "Update" : "Upload"} Data
                  </button>
                </div>
              </div>
            </div>
              ...
              {currentData.map((hewan) => (
                <tr key={hewan.id}>
                  ...
                  <td>
                    <button
                      className="btn-edit"
                      onClick={() => startEdit(hewan)}
                    >
                      Edit
                    </button>
                    <button
                      className="btn-delete"
                      onClick={() => handleDelete(hewan.id)}
                    >
                      Delete
                    </button>
                    <button
                  ...
                  <td>
                    <button
                      ...
                      onClick={() => {
                        ...
                        console.log(
                          "Data lengkap yang akan ditampilkan:",
                          userData
                        );
                        console.log(
                          "Format foto:",
                          typeof p.foto,
                          p.foto ? p.foto.substring(0, 50) : "tidak ada foto"
                        );
      ...
      {popupUser && (
        <div className="popup-overlay" onClick={() => setPopupUser(null)}>
          ...
          >
            ...
            <p>
              <strong>Username Akun:</strong> {popupUser.User?.username || "-"}
            </p>
            <p>
              <strong>Nama Lengkap:</strong> {popupUser.nama}
            </p>
            <p>
              <strong>Jenis Kelamin:</strong> {popupUser.jenisKelamin || "-"}
            </p>
            <p>
              <strong>Tanggal Lahir:</strong> {popupUser.tanggalLahir || "-"}
            </p>
            <p>
              <strong>Email:</strong> {popupUser.User?.email || "-"}
            </p>
            <p>
              <strong>No. HP:</strong> {popupUser.noHp || "-"}
            </p>
            <p>
              <strong>Alamat:</strong> {popupUser.alamat || "-"}
            </p>
            <p>
              <strong>Alasan Adopsi:</strong> {popupUser.alasan || "-"}
            </p>
            <p>
              <strong>Tanggal Pengajuan:</strong>{" "}
              {popupUser.createdAt
                ? new Date(popupUser.createdAt).toLocaleString()
                : "-"}
            </p>
      ...
      {popupHewan && (
        <div className="popup-overlay" onClick={() => setPopupHewan(null)}>
          ...
          >
            ...
            <p>
              <strong>Nama:</strong> {popupHewan.nama}
            </p>
            <p>
              <strong>Jenis:</strong> {popupHewan.jenis}
            </p>
            <p>
              <strong>Umur:</strong> {popupHewan.umur} tahun
            </p>
            <p>
            ...
            <ul>
              {Array.isArray(popupHewan.kondisi)
                ...
                : JSON.parse(popupHewan.kondisi || "[]").map((k, i) => (
                    <li key={i}>{k}</li>
                  ))}
            </ul>
